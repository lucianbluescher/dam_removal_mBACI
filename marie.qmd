...
The figure below illustrates how dam removal via slow drawdown transforms a degraded, low-diversity system into a fully recovered, biodiverse stream ecosystem. It conceptualizes the three stages (before, drawdown, and after) that form the basis of the statistical analysis.”
...
<!-- -   **Unit of observation:** Five random samples with a Surber net (a standardized benthic invertebrate sample) from one site on one sampling occasion -->
<!-- -   **Key variables:** -->
<!--     -   *Outcome:* `richness` (S, taxa richness), `shannon` (H', Shannon-Wiener diversity), `density` (TD, total invertebrate density), `IASPT` (pollution tolerance index) -->
...
|----------------------|-------------|
| **Unit of observation** | Five random samples with a Surber net (a standardized benthic invertebrate sample) from one site on one sampling occasion |
| **Outcome variables** | `richness` (S, taxa richness); `shannon` (H', Shannon-Wiener diversity); `density` (TD, total invertebrate density); `IASPT` (pollution tolerance index) |
| **Treatment**         | `reach_type`: Impact (I1–I4, downstream of dam) vs. Control (C1–C4, undammed tributaries + upstream) |
| **Time**              | `period`: Before (Nov 2017), Drawdown (Oct 2019), After (Nov 2020) |
| **Pre-treatment**   | Before period (B)                 |
| **Post-treatment**   | Drawdown (D) & After (A)    
| **Cleaning decisions**| Reservoir sites (R) excluded — only sampled in the After period, so not usable for before–after comparison |
| **Sample size**       | `r nrow(invert_baci)` samples total — 5 Surber net replicates × 4 sites per group × 3 periods; Artikutza protected catchment, northern Spain (2017–2020) |
...
```{r}
#| label: fig-did
#| fig-cap: "Mean taxa richness by period and reach type. This plot visualizes the Difference-in-Differences (DiD) design by comparing how taxa richness changes over time in impact vs. control sites."
#| fig-width: 7
#| fig-height: 4.5
...
ggplot(did_means, aes(x = period, y = mean_richness,
  ...
  labs(x = "Period", y = "Mean taxa richness (S)", color = "Reach type",
       title = "Mean Taxa Richness Periods and Reach Type") +
  theme_minimal(base_size = 13)
...
-   **What this does:** Visualizes group means across periods to highlight the DiD pattern, but more specifically, whether impact sites exhibit a larger change in taxa richness over time than control sites (i.e., a convergence between groups after treatment).
...
-   **Key assumption (parallel trends):**- The DiD approach assumes that, in the absence of dam removal, impact and control sites would have followed similar trends over time. Because only one pre-treatment period is available, this assumption cannot be directly tested. Instead, we rely on indirect support: control sites are ecologically comparable, multiple response variables show consistent recovery patterns, and stronger effects are observed closer to the dam.
- **Evidence:** - The convergence between impact and control sites from Before to After indicates that impact sites improved more over time in the dams (I1, I2). This differential change is consistent with a causal effect of dam removal, especially given that control sites remain relatively stable and recovery is strongest at sites nearest the dam
------------------------------------------------------------------------
...
# TWO OTHER LME MODELS, DO WE NEED TO ADD THEM?
...
-   **What this does:** Fixed effects for period and reach type are included to control for confounding sources of variation: reach type accounts for baseline differences between control and impacted sites, and period accounts for temporal changes affecting all sites. This ensures that the interaction term isolates the DiD (mBACI) effect (i.e. the change in impacted reaches relative to controls after dam removal).
...
# WHERE IS THE EFFECT OF SITE_CODE? DO WE ADD IT?
```
...
-   Key terms:
    -   `(Intercept)` - Represents the average macro-invertebrate richness in control reaches during the Before period. This serves as the ecological baseline against which all other changes are compared.
    -   `reach_typeI` - Captures the initial difference between impact and control reaches before dam removal. A non-zero value indicates that the two groups were already different prior to intervention (e.g., due to habitat or upstream effects of the dam).
...
-   **What this does:** Checks two core LME assumptions: constant residual variance (homoscedasticity) and approximate normality of residuals
-   **What to look for:**
    -   *Residuals vs. fitted:* Residuals should scatter randomly around zero
...
-   **Identification approach:** Same DiD/mBACI logic; if the drawdown caused recovery, the `period:reach_type` interaction should be significant (or at least directionally consistent) across all metrics, not just one
...
-   Main credibility concern: Parallel trends cannot be established with one observation
...
-   **Normality & homoscedasticity:** Valid; see Figure 3
-   **Random effects structure:** Weak; only 4 sites per group, the random intercept variance is estimated from few clusters, which can lead to inflated p-values.
-   **SUTVA (no spillover):** Valid; C2–C4 are in separate tributaries physically isolated from the drawdown reach; C1 is upstream of the dam

### Sampling & External Validity

-   **Sample:** 8 stream sites in a single protected headwater catchment in Basque Country, Spain; 5 random Surber net samples per site per period
-   **Limited generalizability:** Results apply most directly to sites with similar dam + stream characteristics; dam removal in urban, agricultural, or arid-region streams may show different recovery dynamics
-   **Time scale:** Recovery assessed after only \~1 year post-drawdown. The paper's conclusion of "rapid recovery" may not hold for all taxa, sensitive taxa or rare species may require longer
...
-   **IASPT** depends on accurate taxonomic identification to family level; misidentification error would weaken the signal rather than inflate it
...
-   **No power analysis reported:** With 4 sites per group and 3 time points, the study may not be properly designed to detect moderate-sized effects for some metrics (e.g., IASPT), that is, weaker results could represent a false negative for these metrics.
