...
mod_richness <- lmer(richness ~ period * reach_type + (1 | site_code), # this accounts for non-independence of replicates within sites (5 observations per site per occasion)
                     data = invert_baci,
                     REML = TRUE)
...
tidy(mod_richness, effects = "fixed", conf.int = TRUE) |>
  kable(digits = 3,
        col.names = c("Effect", "Term", "Estimate", "SE", "Statistic", "CI Lower", "CI Upper"))
```
